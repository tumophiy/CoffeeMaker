class CoffeeMachine
  ...
  def cook(item_name)
    if @happiness > 0
      ...
      return "your barista is dead tired, sorry for not helping you"
    end
    if @water_capacity > 0
      @water_capacity -= 0.1
    else 
      return "no water, please fill container"
    end
    if @menu.include?(item_name.downcase)
      puts boil_water
      "Here you go, your #{item_name})"
    else 
      "We don't do #{item_name} now( Consider other items in our menu. You can get full list calling .what_can_you_serve method"
    end
  end
...
john = CoffeeMachine.new({ name: 'john', water_capacity: 0.2, menu: ['capuchino', 'latte'] })
